...
public class BellPepperMedleyBlock extends FeastBlock {
    ...
    public static final IntProperty MEDLEY_SERVINGS = IntProperty.of("servings", 0, 3);
    ...
    public BellPepperMedleyBlock(Settings settings, List<Supplier<Item>> peppers) {
        super(settings, peppers.get(0), true);
    ...
    public @NotNull ItemStack getServingItem(BlockState state) {
        ...
        return new ItemStack(peppers.get(state.get(getServingsProperty()) - 1).get());
    }
    ...
    public @NotNull VoxelShape getOutlineShape(BlockState state, @NotNull BlockView level, @NotNull BlockPos pos, @NotNull ShapeContext context) {
        return SHAPES.get(state.get(FACING))[state.get(getServingsProperty())];
    }
    ...
    private static Map<Direction, VoxelShape[]> buildShapes() {
        VoxelShape platter = Block.createCuboidShape(1.0D, 0.0D, 2.0D, 15.0D, 3.0D, 14.0D);
        ...
        shapes[0] = platter;
        shapes[1] = VoxelShapes.union(platter, backLeft);
        shapes[2] = VoxelShapes.union(platter, backLeft, backRight);
        shapes[3] = VoxelShapes.union(platter, backLeft, backRight, front);
    ...
    private static VoxelShape pepper(double x, double z) {
        return VoxelShapes.union(
                Block.createCuboidShape(x, 3.0D, z, x + 5.0D, 8.0D, z + 5.0D),
                Block.createCuboidShape(x + 1.0D, 8.0D, z + 1.0D, x + 4.0D, 9.0D, z + 4.0D));
    }
    ...
    private static VoxelShape rotateY(VoxelShape shape) {
        ...
        shape.forEachBox((minX, minY, minZ, maxX, maxY, maxZ) ->
                result[0] = VoxelShapes.combine(result[0],
                        VoxelShapes.cuboid(1.0D - maxZ, minY, minX, 1.0D - minZ, maxY, maxX), BooleanBiFunction.OR));
