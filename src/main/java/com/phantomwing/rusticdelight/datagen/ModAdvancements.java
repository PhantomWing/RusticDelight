...
public class ModAdvancements extends ModAdvancementProvider {
    private static final ResourceLocation BACKGROUND =
            new ResourceLocation("textures/block/dirt_path_top.png");
    ...
    protected void generate(HolderLookup.Provider registries) {
        ...
        Advancement root = save(Advancement.Builder.advancement()
        ...
        ICondition batterEnabled = new OrCondition(
        ...
        Advancement batter = obtainMatching(root, "batter", ModItems.BATTER.get(),
    ...
    private void cotton(Advancement root) {
        ConfigBooleanCondition enabled = new ConfigBooleanCondition(Configuration.ENABLE_COTTON_ID);

        Advancement cotton = obtain(root, "cotton", ModItems.COTTON_BOLL.get(),
                FrameType.TASK, enabled, ModItems.COTTON_BOLL.get());
        ...
        save(Advancement.Builder.advancement()
                        ...
                        .addCriterion("cooking_oil", InventoryChangeTrigger.TriggerInstance.hasItems(
                                ItemPredicate.Builder.item().of(ModItems.COOKING_OIL.get()).build())),
        ...
        save(Advancement.Builder.advancement()
                        ...
                        .addCriterion("string_from_cotton", RecipeCraftedTrigger.TriggerInstance.craftedItem(
                                new ResourceLocation(RusticDelight.MOD_ID, "string_from_cotton_boll"))),
                "main/string", enabled);
    }

    private void bellPepper(Advancement root) {
        ConfigBooleanCondition enabled = new ConfigBooleanCondition(Configuration.ENABLE_BELL_PEPPERS_ID);

        Advancement pepper = obtain(root, "bell_pepper", ModItems.BELL_PEPPER_RED.get(),
                FrameType.TASK, enabled,
        ...
        obtainAll(pepper, "all_bell_peppers", ModItems.BELL_PEPPER_PURPLE.get(),
                FrameType.GOAL, enabled,
                ModItems.BELL_PEPPER_GREEN.get(), ModItems.BELL_PEPPER_YELLOW.get(), ModItems.BELL_PEPPER_RED.get(),
                ModItems.BELL_PEPPER_ORANGE.get(), ModItems.BELL_PEPPER_WHITE.get(), ModItems.BELL_PEPPER_PINK.get(),
                ModItems.BELL_PEPPER_BLUE.get(), ModItems.BELL_PEPPER_PURPLE.get(), ModItems.BELL_PEPPER_BLACK.get());

        Advancement stuffed = obtain(pepper, "stuffed_bell_pepper", ModItems.STUFFED_BELL_PEPPER_RED.get(),
                FrameType.TASK, enabled,
        ...
        save(Advancement.Builder.advancement()
                        ...
                        .addCriterion("rice_roll_royale", InventoryChangeTrigger.TriggerInstance.hasItems(
                                ItemPredicate.Builder.item().of(ModItems.RICE_ROLL_ROYALE.get()).build())),
    ...
    private void coffee(Advancement root) {
        ConfigBooleanCondition enabled = new ConfigBooleanCondition(Configuration.ENABLE_COFFEE_ID);

        Advancement beans = obtain(root, "coffee_beans", ModItems.COFFEE_BEANS.get(),
                FrameType.TASK, enabled, ModItems.COFFEE_BEANS.get());

        Advancement roasted = obtain(beans, "roasted_coffee_beans", ModItems.ROASTED_COFFEE_BEANS.get(),
                FrameType.TASK, enabled, ModItems.ROASTED_COFFEE_BEANS.get());

        Advancement coffee = obtain(roasted, "coffee", ModItems.COFFEE.get(),
                FrameType.TASK, enabled, ModItems.COFFEE.get());
        ...
        obtain(beans, "golden_coffee_beans", ModItems.GOLDEN_COFFEE_BEANS.get(),
                FrameType.TASK, enabled, ModItems.GOLDEN_COFFEE_BEANS.get());
    }
     ...
     */
    private Advancement obtainMatching(Advancement parent, String name, Item icon,
                                       ItemPredicate.Builder match, ICondition... conditions) {
        return save(Advancement.Builder.advancement()
                        .parent(parent)
                        .display(icon, title(name), description(name), null, FrameType.TASK, true, true, false)
                        .addCriterion(name, InventoryChangeTrigger.TriggerInstance.hasItems(match.build())),
                "main/" + name, conditions);
    }

    /** An advancement granted by picking up any one of {@code items}. */
    private Advancement obtain(Advancement parent, String name, Item icon,
                               FrameType type, ConfigBooleanCondition condition, ItemLike... items) {
     ...
     */
    private Advancement obtainAll(Advancement parent, String name, Item icon,
                                  FrameType type, ConfigBooleanCondition condition, ItemLike... items) {
        Advancement.Builder builder = Advancement.Builder.advancement()
                .parent(parent)
                .display(icon, title(name), description(name), null, type, true, true, false);
