...
import com.phantomwing.rusticdelight.loot.ConfigurableReplaceItemModifier;
...
public class ModGlobalLootModifiersProvider extends GlobalLootModifierProvider {
    ...
    // Forge 1.20.1 cannot attach load conditions to a loot modifier, so the master loot toggle and
    // the crop family toggle are checked at runtime inside ConfigurableReplaceItemModifier instead.
    private void replaceInChest(String name, String lootTable, Item removed, Item added, int count, String settingId) {
        add(name, new ConfigurableReplaceItemModifier(
                new LootItemCondition[]{defaultLootTable(lootTable)}, removed, added, count, settingId));
    }

    private void replaceInChest(String name, String lootTable, Item removed, Item added, int count, String settingId, float chance) {
        add(name, new ConfigurableReplaceItemModifier(new LootItemCondition[]{
                defaultLootTable(lootTable),
                LootItemRandomChanceCondition.randomChance(chance).build(),
        }, removed, added, count, settingId));
    }
